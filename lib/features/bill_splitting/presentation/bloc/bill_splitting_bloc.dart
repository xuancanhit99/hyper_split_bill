...
// TODO: Import other use cases (CreateBill, DeleteBill, etc.) when needed
...
class BillSplittingBloc extends Bloc<BillSplittingEvent, BillSplittingState> {
  ...
  Future<void> _onProcessOcr(
    ...
    final failureOrText = await _processBillOcrUseCase(
      imageFile: event.imageFile,
      prompt: event.prompt,
    );
    failureOrText.fold(
      (failure) => emit(BillSplittingOcrFailure(failure.message)),
      (text) => emit(BillSplittingOcrSuccess(extractedText: text)),
    );
  }
